...
public class CreateProductProfileValidator : AbstractValidator<CreateProductProfileRequest>
{
        ...
        ILogger<CreateProductProfileValidator> logger)
    {
        _context = context;
        _logger = logger;
        _businessRules = new ProductBusinessRules(context, 
            logger as ILogger<ProductBusinessRules> ?? 
            LoggerFactory.Create(b => b.AddConsole()).CreateLogger<ProductBusinessRules>());
